...
The 6510 CPU (6502 variant) in the C64 has a built-in I/O port at $0000-$0001 for memory banking. Otherwise identical to standard 6502. This summary blends the CPU sections from the Commodore 64 Programmer's Reference Guide (Chapter 5 + Appendix L) and Mapping the Commodore 64 (Chapters 1–2) so both canonical etexts are acknowledged.

### Zero page layout

Zero page ($0000-$00FF) is heavily used by the KERNAL. Location $0000 is the data-direction register for the 6510 I/O port, while $0001 controls the LORAM, HIRAM and CHAREN lines that select BASIC/KERNAL ROMs, I/O, or character ROM. Write-protect this area unless you bank out the ROMs: $02-$8F are occupied by BASIC/KERNAL workspace, $90 holds `ST`, $93-$98 point to tape buffers, $A0-$A2 host the TI/TI$ clock, and $B2-$B3 store the start of the BASIC program. User code may use $FB-$FE for scratch variables because the system seldom touches them (Mapping cap. 1).
...
On the C64, BASIC uses the stack for FOR/NEXT (18 bytes per loop) and GOSUB (5 bytes per call), and it keeps `PEEK(256+PEEK(9))` in mind to avoid overflow. The range $0100-$013E also hosts floating-point conversion helpers and tape error logs, so leave room for the stack to grow; if you need temporary storage, consider your own zero-page area at $FB-$FE instead (Mapping cap. 2).
...
| 3 | D | Decimal | BCD mode (supported; use SED/CLD) |
...
## Hardware vectors and KERNAL entry points

The last six bytes of memory ($FFFA-$FFFF) hold the system vectors: $FFFA points to the NMI handler, $FFFC to the cold start/RESET routine, and $FFFE to the IRQ/BRK handler. Calling `SYS` from BASIC ultimately pushes A/X/Y/P via the $030C-$030F area before jumping into the ROM, so save register state if you call the same vector from custom ML (Mapping cap. 7). These vectors are the glue between the 6502, KERNAL, and BASIC loops.
